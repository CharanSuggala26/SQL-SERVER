...
class DBModel(object):
    def __init__(self):
        ...
        self.loaded_entities = []
        self.config = Configuration()
        self.conn = pyodbc.connect("DRIVER={ODBC Driver 17 for SQL Server};SERVER=SAICHARAN;DATABASE=TextToSql;Trusted_Connection=yes;")
    ...
    def load_db_model(self):
        ...
        # Load foreign keys and relationships
        cursor.execute(self.config.get_FK_sql_query())
        for row in cursor:
            self.relationships.append(Relationship(row.parent_table, row.refrenced_table, row.parent_table_col, row.referenced_table_col))

            # Build entity graph
            for a, b in [(row.parent_table, row.refrenced_table), (row.refrenced_table, row.parent_table)]:
                entry = next((en for en in self.entity_graph if en[0] == a), None)
                if entry:
                    if b not in entry[1]:
                        entry[1].append(b)
                else:
                    self.entity_graph.append((a, [b]))
        ...
        # Load entity data and lemmatize
        for entity_to_load in self.config.get_entitites_to_load():
            entity_load_query = f"SELECT DISTINCT {entity_to_load['column']} FROM {entity_to_load['entity']}"
            cursor.execute(entity_load_query)
            entity_data = (entity_to_load["entity"], [])
            for row in cursor:
                value = str(row[0])
                entity_data[1].append(value)

                # Lemmatize using nlp pipeline
                doc = self.nlp(value)
                for token in doc:
                    lemma = token.lemma_
                    if lemma != value:
                        entity_data[1].append(lemma)
